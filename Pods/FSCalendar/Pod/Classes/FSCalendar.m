...
-(void)setSelectedDateArray:(NSArray *)selectedDateArray
{
    for(int i = 0; i<selectedDateArray.count; i++){
        if (![self isDateInRange:selectedDateArray[i]]) {
            [NSException raise:@"selectedDate out of range" format:nil];
        }
        [self SelectedDate:selectedDateArray[i] animate:NO];
        
    }
  
}

- (void)SelectedDate:(NSDate *)selectedDate animate:(BOOL)animate
{
    ...
    FSCalendarCell *cell = (FSCalendarCell *)[self.collectionView cellForItemAtIndexPath:selectedIndexPath];
//    cell.backgroundColor = [UIColor grayColor];
//    cell.hidden = NO;
//    [cell showAnimation];
//    _selectedDate = [self dateForIndexPath:selectedIndexPath];
//    [self didSelectDate:_selectedDate];
//    cell.cellStyle = FSCalendarCellStateSelected;
