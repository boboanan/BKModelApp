...
@interface MyProfileController ()<UzysAssetsPickerControllerDelegate>
...
{
    if(!_listArray){
        _listArray = @[@"头像",@"昵称",@"身份",@"评价",@"商家类型",@"最近活动"];
    }
    return _listArray;
}
...
{
    if(!_listData){
        _listData = @[@"更改头像",@"比尔盖茨",@"商家",@"好评",@"个人商家",@"查看"];
    }
    return _listData;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.detailTextLabel.text = self.listData[indexPath.row];
    if(indexPath.row == 0){
        cell.imageView.image = self.image;
    }
    if(indexPath.row == 1){
       cell.detailTextLabel.text = self.account.name;
        BKLog(@"%@",self.account.name);
    }
    if(indexPath.row != 0){
